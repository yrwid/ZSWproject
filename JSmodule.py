...
def exit(time, cmax, delta,l_maszyn, *nr):
    el_pos_x = 30 + time * 750/(cmax)
    el_pos_y = 93-20*(4-l_maszyn)
    lst = [ellipse(cx=el_pos_x, cy=str(el_pos_y)+'%', rx=10*len(nr), ry=10)]
    pom = ''
    i=0
    for key in nr:
        ...
        if(i!=len(nr)):
            pom = pom + ','
    lst.append(text(x = el_pos_x, y =str(el_pos_y+0.5)+'%', tekst = pom))
    lst.append(tag('svg', x = el_pos_x-5, y = str(el_pos_y-6)+'%'))
...
def solveJS(problem_info, method = 'all', show_input = 'html', show_result = 'gantt'):
    
    if(isinstance(problem_info, int)):
        random.seed(problem_info)
        p = generate()
    else:
        p = userInput()
    ...
    if(method == 'all'):
        for met in methods:
            schedule, conflicts, arrival_route = solution(p, met)
            objectives = stats(schedule, p)
            
            if(show_result == 'gantt'):
                drawGantt(schedule, conflicts, arrival_route, objectives[0])
                print("---------------------------------------------------------------------------------------------------------------")
            else:
                print("schedule: ", schedule)
                print("conflicts: ", conflicts)
                print("arrival_route: ", arrival_route)
                print("---------------------------------------------------------------------------------------------------------------")
                print("\n")
    else:
        schedule, conflicts, arrival_route = solution(p, method)
        objectives = stats(schedule, p)
...
def drawGantt(sch,conf,arr,Cmax):
    delta = ceil(Cmax/15) #15 - maximum amount of intervals
    Cmax = ceil(Cmax / delta) * delta
    result = tag('svg', height=125*MACHINES_NUM+100, width=830)
    result = result + tag('svg', height=600, width=830)
   ...
   # print(skipList)
    ...
    for entity in changeFormat:
        if (len(entity) == 4):
            result = result + arrival(entity[0],entity[1],Cmax,delta,entity[3])
        else:
            result = result + arrival(entity[0],entity[1],Cmax,delta,entity[3], entity[4])
    
    #print rectancle tasks
    for task in sch:
        result = result + drawTask(task[0],task[2],task[3],task[4]-task[3],Cmax,delta)
              
    #print axies  need to implement finding exits points          
    for i in range(MACHINES_NUM):
        result = result + axis(i+1,Cmax,delta)
    ...
    for i in range(len(exits)):
        if(type(exits[i][2]) == type(1)):
            result = result + exit(exits[i][0],Cmax,delta,MACHINES_NUM,exits[i][2])
        if(type(exits[i][2]) == type([1,2])):
            result = result + exit(exits[i][0],Cmax,delta,MACHINES_NUM,exits[i][2][0], exits[i][2][1])
              
    for i in range((len(conf))):
        result = result + conflict(conf[i][0],conf[i][1],Cmax,delta)
    
    result = result + tag('/svg')
    result = result + tag('/svg')
    display(HTML(result))
...
#seed = "user inpt"
#seed = 1
#solveJS(seed,'all','html','gantt')
#solveJS(seed,'all','raw','raw')